...
class HomeFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        listHome.apply {
            layoutManager = GridLayoutManager(context, resources.getInteger(R.integer.home_max_span))
            adapter = homeAdapter
        }
        (listHome.layoutManager as GridLayoutManager).spanSizeLookup = object: GridLayoutManager.SpanSizeLookup(){
            override fun getSpanSize(position: Int): Int = resources.getInteger(homeAdapter.getItemSpan(position))
        }
        homeAdapter.addMovies(mockNewReleases(), mockUpcoming(), mockPopular())
    }

    private fun mockNewReleases(): List<Movie> {
        return listOf(
            Movie(
                320288,
                "Dark Phoenix",
    ...
    private fun mockUpcoming(): List<Movie> {
        return listOf(
            Movie(
                420817,
                "Aladdin",
    ...
    private fun mockPopular(): List<Movie> {
        return listOf(
            Movie(
                320288,
                "Dark Phoenix",
